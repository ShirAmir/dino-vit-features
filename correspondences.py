...
def draw_correspondences(points1: list[tuple[float, float]], points2: list[tuple[float, float]],
                         image1: Image, image2: Image) -> tuple[plt.Figure, plt.Figure]:
    """
    ...
    if num_points > 15:
        ...
        cmap = ListedColormap(["red", "yellow", "blue", "lime", "magenta", "indigo", "orange", "cyan", "darkgreen",
                                "blueviolet", "black", "white", "chocolate", "maroon", "gray"])
    colors = np.array([cmap(x) for x in range(n_clusters)])
    radius1, radius2 = 8, 1
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for pair_dir in tqdm(pair_dirs):
            ...
            image1_batch, image1_pil = extractor.preprocess(curr_images[0], args.load_size)
            descriptors1 = extractor.extract_descriptors(image1_batch.to(device), args.layer, args.facet, args.bin)
            num_patches1, load_size1 = extractor.num_patches, extractor.load_size
            image2_batch, image2_pil = extractor.preprocess(curr_images[1], args.load_size)
            num_patches2, load_size2 = extractor.num_patches, extractor.load_size
            descriptors2 = extractor.extract_descriptors(image2_batch.to(device), args.layer, args.facet, args.bin)
            ...
            fg_mask1 = saliency_map1 > args.thresh
            fg_mask2 = saliency_map2 > args.thresh
            ...
            temp_arange1 = np.arange(num_patches1[0] * num_patches1[1])
            temp_arange2 = np.arange(num_patches2[0] * num_patches2[1])
            # apply k-means on a concatenation of a pairs descriptors.
            all_keys_together = np.concatenate((bb_descs1, bb_descs2), axis=1)
            n_clusters = min(args.num_pairs, len(all_keys_together))  # if not enough pairs, show all found pairs.
            ...
            # converting and saving points in image coordinates
            source_file = open(curr_save_dir / "correspondence_A.txt", "w")
            target_file = open(curr_save_dir / "correspondence_Bt.txt", "w")
            points1, points2 = [], []
            for y1, x1, y2, x2 in zip(img1_y_to_show, img1_x_to_show, img2_y_to_show, img2_x_to_show):
                ...
                source_file.write(f'{int(y1_show)}, {int(x1_show)}\n')
                target_file.write(f'{int(y2_show)}, {int(x2_show)}\n')
            source_file.close()
            target_file.close()
